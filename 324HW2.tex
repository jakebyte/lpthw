...
\textit{X} is a Pascal RV, therefore its expected value $ E[X] = \frac{k}{p} $ where \textit{p} = \textit{q} and textit{k} is the number of successes needed, 3. Therefore: $$E[X] = \frac{3}{q}$$
...
\end{tabular} \\
\newline
 Each is equally likely. The first jump is successful for 2 of the 3 possibilities, therefore the probability that the first jump was a success is $$P(\{first {}jump{} success \}|X = 4) =  \frac{2}{3} $$.
\end{itemize}
